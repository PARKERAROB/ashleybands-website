...
export const COORDINATION_KINDS = ["update", "opportunity", "assessment", "recommendation", "reported_decision", "volunteer_response"];
export function teamProjection(state) {
  return state.records.filter((r) => r.team_visible === true).map((r) => ({
    id: r.id, title: r.title, value: r.value, kind: r.kind, domain: r.domain,
    status: r.status, updated_at: r.updated_at,
    attributed_to: r.attributed_to || null, occurred_on: r.occurred_on || null,
...
export function textField(value, name, max = 2000, optional = false) {
  ...
  if (
    /(?:password|passcode|\bpin\b|api[_ -]?key|secret|session[_ -]?token)\s*[:=]\s*\S+|-----BEGIN [\w ]*PRIVATE KEY|\b(?:sk|sb_secret|cw)_[a-zA-Z0-9_-]{12,}/i.test(
      text,
    )
  )
...
export function applyWorkspaceAction(
  ...
  if (actor.access === "viewer") throw new WorkspaceError("This is read-only access.", 403);
  const id = randomUUID();
  const source = textField(command.source, "source", 1000);
  switch (command.action) {
    case "coordination.save": {
      if (!actor.domains.includes("coordination")) throw new WorkspaceError("Coordination access required.", 403);
      const existing = command.id ? find(state.records, command.id) : null;
      if (existing && (existing.domain !== "coordination" || !COORDINATION_KINDS.includes(existing.kind)))
        throw new WorkspaceError("Use the source owner's review for this record.", 403);
      if (existing && existing.version !== command.base_version) throw new WorkspaceError("This entry changed. Refresh and compare.", 409);
      const kind = oneOf(command.kind, COORDINATION_KINDS, "entry type");
      const attributed = ["reported_decision", "volunteer_response"].includes(kind);
      const occurred = textField(command.occurred_on || "", "date", 10, !attributed);
      if (occurred && (!/^\d{4}-\d{2}-\d{2}$/.test(occurred) || !Number.isFinite(Date.parse(occurred)) || new Date(occurred).toISOString().slice(0, 10) !== occurred)) throw new WorkspaceError("Use a valid date.");
      const row = {
        ...(existing || { id, owner_id: actor.id, created_by: actor.id, version: 0 }),
        kind, domain: "coordination", title: textField(command.title, "title", 160),
        value: textField(command.value, "update"), source,
        status: attributed ? "reported" : "current",
        attributed_to: textField(command.attributed_to || "", "person who decided or responded", 160, !attributed),
        occurred_on: occurred,
        response: kind === "volunteer_response" ? oneOf(command.response, ["requested", "accepted", "declined", "waiting", "completed"], "volunteer response") : null,
        team_visible: command.team_visible === true,
      };
      stamp(row, actor, now);
      if (existing) state.records[state.records.indexOf(existing)] = row;
      else state.records.push(row);
      break;
    }
    case "record.share": {
      const row = find(state.records, command.id);
      if (!actor.is_primary) owns(row, actor);
      if (row.version !== command.base_version) throw new WorkspaceError("This entry changed. Refresh and compare.", 409);
      row.team_visible = command.team_visible === true;
      row.share_source = source;
      stamp(row, actor, now);
      break;
    }
    case "record.create": {
    ...
    case "record.confirm": {
      const row = find(state.records, command.id);
      if (COORDINATION_KINDS.includes(row.kind)) throw new WorkspaceError("Coordination reports remain attributed reports; edit them directly.");
      owns(row, actor);
    ...
    case "proposal.create": {
      ...
      if (COORDINATION_KINDS.includes(row.kind)) throw new WorkspaceError("Edit coordination entries directly.");
