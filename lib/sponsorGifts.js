import { normalizeGiftPurpose, giftTermsVersion, sameGiftRequest, CARNEGIE_CAMPAIGN } from "@/lib/sponsorCampaigns.mjs";
...
export async function createPendingGift({
  ...
  let version;
  try {
    purpose = normalizeGiftPurpose({ campaign_code: campaignCode, gift_kind: giftKind });
    version = giftTermsVersion(purpose.campaignCode, termsVersion);
    targets = await resolveGiftTargets({ attributionToken, businessName });
    // Staff-recorded Carnegie credit from a check memo (#103). Never combined with a signed link.
    if (staffStudentId) {
      if (attributionToken || purpose.campaignCode !== CARNEGIE_CAMPAIGN) throw new Error("Student credit here is for Carnegie offline gifts only.");
      const { data: student, error: studentError } = await supabaseAdmin
        .from("portal_students")
        .select("id")
        .eq("id", staffStudentId)
        .eq("status", "active")
        .maybeSingle();
      if (studentError) throw new Error("The student could not be verified.");
      if (!student) throw new Error("Choose an active student from the list.");
      targets = { ...targets, portalStudentId: student.id };
    }
  } catch (error) {
  ...
  const insert = {
    ...
    campaign_code: purpose.campaignCode,
    gift_kind: purpose.giftKind,
    gift_terms_version: version,
    ...(purpose.campaignCode === CARNEGIE_CAMPAIGN ? { fmv_cents: 0 } : {}),
    method,
    status: "pending",
  ...
  const fields = "id, invoice_id, amount_cents, business_name, tier, method, status, paypal_order_id, campaign_code, gift_kind, gift_terms_version, portal_student_id, family_id, prospect_id, business_id";
  ...
  if (error?.code === "23505") {
    const { data: existing } = await supabaseAdmin
      ...
      .maybeSingle();
    const sameRequest = sameGiftRequest(existing, insert);
