  ...
  try {
    unzipSync(bytes, {
      filter(meta) {
        ...
        names.add(meta.name);
        if (
          /vbaProject|embeddings\/|activeX\/|externalLinks\/|customXml\/|\.bin$|\.exe$|\.js$/i.test(
            meta.name,
          )
        )
          throw new WorkspaceError(
            "Remove macros, embedded objects, custom XML, or external data connections before uploading.",
          );
        return false;
      },
    });
