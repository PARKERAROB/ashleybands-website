...
// pdf.js builds one DOMMatrix when it loads. The optional @napi-rs/canvas package supplies it in
// Node, but it is absent from the Vercel function bundle (#115). Text extraction never renders, so
// an inert stand-in is enough, and it is installed only when no real DOMMatrix exists.
export function ensureTextOnlyDomMatrix(scope = globalThis) {
  if (typeof scope.DOMMatrix === "function") return false;
...
// In Node, pdf.js runs its worker in-process and otherwise imports "pdf.worker.mjs" by a relative
// path at runtime. That file is missing from the bundled Vercel function (#115). pdf.js first checks
// globalThis.pdfjsWorker, so load the worker through a static import the bundler includes.
export async function ensureInProcessPdfWorker(scope = globalThis) {
  if (scope.pdfjsWorker?.WorkerMessageHandler) return false;
...
export async function parseAttendanceRegisterPdf(input) {
  ...
  ensureTextOnlyDomMatrix();
  await ensureInProcessPdfWorker();
