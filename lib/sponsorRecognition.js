import { CARNEGIE_CAMPAIGN, giftCampaignLabel, giftChangeTerms } from "@/lib/sponsorCampaigns.mjs";
...
export function renderReceiptEmail({
  ...
  giftKind = "sponsorship",
  receivedAt,
  termsVersion
}) {
  const carnegie = campaignCode === CARNEGIE_CAMPAIGN;
  const subject = carnegie ? `Your Ashley Carnegie trip gift receipt — ${receiptNo}` : `Your Ashley Bands sponsorship receipt — ${receiptNo}`;
  const purposeLine = `Gift purpose: ${giftCampaignLabel(campaignCode)}`;
  const receivedLine = receivedAt ? `Received: ${new Date(receivedAt).toLocaleDateString("en-US", { timeZone: "America/New_York", year: "numeric", month: "long", day: "numeric" })}` : "";
  const donorLabel = giftKind === "donation" ? "Donor name" : "Sponsor recognition name";
  const recipientOrg = carnegie ? "Ashley High School Band Boosters" : SPONSOR_CONTACT.boosterOrg;
  const thanks = carnegie ? "Thank you for supporting Ashley Bands’ Carnegie Hall performance trip." : "Thank you for sponsoring the Bands of Ashley High School.";
  const fmvLine =
    fmvCents > 0
      ? `Fair market value of benefits received: ${dollars(fmvCents)}\nTax-deductible portion of your gift: ${dollars(deductibleCents)}`
      : carnegie ? `No goods or services were provided in exchange for this gift. Eligible charitable contribution: ${dollars(deductibleCents)}, to the extent allowed by law.` : `This gift was acknowledgment-only (program/website listing). The full amount is tax-deductible: ${dollars(deductibleCents)}`;

  const recognitionLine = giftKind === "donation" && !listedOnSite ? "Thank you for helping Ashley students take part in this experience." : listedOnSite
  ...
  const text = [
    thanks,
    ...(carnegie ? [purposeLine, receivedLine, giftChangeTerms(termsVersion)] : []),
    "",
    `Payment received from: ${payerName || "Sponsor"}`,
    `${donorLabel}: ${businessName}`,
    ...
    fmvLine,
    "",
    `${recipientOrg} is a registered 501(c)(3) educational nonprofit. EIN ${SPONSOR_CONTACT.ein}. No goods or services were provided in exchange for this contribution except as noted above.`,
    "",
    recognitionLine,
    "",
    `With gratitude,`,
  ...
  const html = [
    `<p>${esc(thanks)}</p>`,
    ...(carnegie ? [`<p>${esc(purposeLine)}<br/>${esc(receivedLine)}</p><p>${esc(giftChangeTerms(termsVersion))}</p>`] : []),
    ...
    `<tr><td style="padding:2px 12px 2px 0;color:#6f675a">${donorLabel}</td><td><strong>${esc(businessName)}</strong></td></tr>`,
    ...
    fmvCents > 0
      ...
      : carnegie ? `<p>No goods or services were provided in exchange for this gift. Eligible charitable contribution: <strong>${dollars(deductibleCents)}</strong>, to the extent allowed by law.</p>` : `<p style="font-size:14px">This gift was acknowledgment-only (program/website listing). The full amount is tax-deductible: <strong>${dollars(deductibleCents)}</strong>.</p>`,
    `<p style="color:#6f675a;font-size:13px">${esc(recipientOrg)} is a registered 501(c)(3) educational nonprofit. EIN ${esc(SPONSOR_CONTACT.ein)}. No goods or services were provided in exchange for this contribution except as noted above.</p>`,
    giftKind === "donation" && !listedOnSite ? `<p>Thank you for helping Ashley students take part in this experience.</p>` : listedOnSite
...
export async function confirmGift(
  ...
  const { data: gift, error } = await supabaseAdmin
    .from("sponsor_gifts")
    .select(
      "id, campaign_code, gift_kind, gift_terms_version, business_id, business_name, amount_cents, method, status, tier, fmv_cents, payer_name, payer_email, receipt_number, receipt_sent_at, recognition_status, listed_on_site, business:businesses(name_display)"
    )
    .eq("id", giftId)
    .maybeSingle();
  ...
  // Personal campaign donations never auto-publish at offline confirmation.
  if (gift.campaign_code === CARNEGIE_CAMPAIGN && gift.gift_kind === "donation") listOnSite = false;
  ...
  if (sponsorRecognitionLive() && recipient) {
    try {
      const email = renderReceiptEmail({
        businessName,
        campaignCode: gift.campaign_code,
        giftKind: gift.gift_kind,
        termsVersion: gift.gift_terms_version,
        receivedAt: now,
        giftId: gift.id,
