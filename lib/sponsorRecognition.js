import { CARNEGIE_CAMPAIGN, giftCampaignLabel } from "@/lib/sponsorCampaigns.mjs";
...
export function renderReceiptEmail({
  ...
  const text = [
    thanks,
    ...(carnegie ? [purposeLine, receivedLine] : []),
    "",
  ...
  const html = [
    `<p>${esc(thanks)}</p>`,
    ...(carnegie ? [`<p>${esc(purposeLine)}<br/>${esc(receivedLine)}</p>`] : []),
...
export async function confirmGift(
  ...
  const { data: gift, error } = await supabaseAdmin
    .from("sponsor_gifts")
    .select(
      "id, campaign_code, gift_kind, business_id, business_name, amount_cents, method, status, tier, fmv_cents, payer_name, payer_email, receipt_number, receipt_sent_at, recognition_status, listed_on_site, business:businesses(name_display)"
    )
    .eq("id", giftId)
    .maybeSingle();
