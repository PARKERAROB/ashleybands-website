...
import { buildNameSuggestions, normalizeName, sourceNameKey, studentNameKey } from "@/lib/schoolAttendanceMatching.mjs";
import {
...
export async function buildSchoolAttendancePreview(bytes) {
  ...
  const nameSuggestions = buildNameSuggestions(context.students, parsed.students);
  const rows = parsed.students.map((sourceStudent) => {
    ...
    const suggestedId = nameSuggestions.get(sourceStudent.sourceStudentNumber) || null;
