export const CARNEGIE_AGREEMENT_VERSION = "2026-09-03-v3";
export const CARNEGIE_LEGACY_AGREEMENT_VERSION = "2026-09-01-v1";
export const CARNEGIE_V2_AGREEMENT_VERSION = "2026-09-03-v2";
...
export const CARNEGIE_DEPOSIT_CHOICES = Object.freeze([
  { value: "pay_now", label: "Pay the $50 conditional deposit now" },
  { value: "cannot_pay_now", label: "Cannot pay the $50 conditional deposit at this time" },
]);
...
export function carnegieDepositChoiceLabel(value, agreementVersion = CARNEGIE_AGREEMENT_VERSION, response = "") {
  if (agreementVersion !== CARNEGIE_AGREEMENT_VERSION) {
    return response === "serious_yes"
      ? "$50 deposit requested under the earlier form"
      : "No separate deposit choice on the earlier form";
  }
  return CARNEGIE_DEPOSIT_CHOICES.find((option) => option.value === value)?.label || "No deposit choice";
}

export function carnegieSubmissionRequestsDeposit(submission) {
  if (!submission) return false;
  if (submission.agreement_version === CARNEGIE_AGREEMENT_VERSION) {
    return submission.response !== "no" && submission.deposit_choice === "pay_now";
  }
  return submission.response === "serious_yes";
}
