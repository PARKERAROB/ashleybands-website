...
const CONTACT_QUERY_BATCH_SIZE = 100;
...
async function guardianRecipients(studentIds, client) {
  ...
  // A current roster can have hundreds of connected adults. Sending all person
  // ids in one PostgREST `.in()` filter can exceed HTTP header limits, and the
  // old unchecked failure silently reduced the audience to students only.
  const contacts = [];
  for (let i = 0; i < personIds.length; i += CONTACT_QUERY_BATCH_SIZE) {
    const { data, error } = await client
      .from("portal_contact_methods")
      .select("person_id, value_display, value_normalized, verification_status")
      .in("person_id", personIds.slice(i, i + CONTACT_QUERY_BATCH_SIZE))
      .eq("contact_type", "email");
    if (error) throw new Error(error.message || "Guardian email lookup failed.");
    contacts.push(...(data || []));
  }
