...
async function guardianRecipients(studentIds, client) {
  ...
  const personIds = uniqueIds(links, "person_id");
...
export async function resolveAudience(filter, axis = "guardians", client = supabaseAdmin) {
  ...
  if (!studentIds.length) {
    return { recipients: [], count: 0, studentCount: 0 };
  }
  ...
  return { recipients, count: recipients.length, studentCount: studentIds.length };
}
