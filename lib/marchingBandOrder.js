...
export function attendanceSectionForStudent({ role, instrument }) {
  if (!role && /\b(front ensemble|pit|mallets?|synth|rack)\b/i.test(String(instrument || ""))) {
    return "Front Ensemble";
  }
  if (!role && /\b(battery|snare|bass drum|quads?|tenors?)\b/i.test(String(instrument || ""))) {
    return "Battery Percussion";
  }
