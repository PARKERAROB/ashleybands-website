...
// Last day of the window: `days` days starting today, stretched to the following
// Sunday when `throughWeekend` is set so the coming weekend is never cut off (#143).
export function windowEnd(today, { days = 7, throughWeekend = false } = {}) {
  let last = addDays(today, days - 1);
  while (throughWeekend && weekday(last) !== 0) last = addDays(last, 1);
  return last;
}

// Upcoming events grouped by day from today through `windowEnd` (band timezone).
// Timed events that have already finished today are dropped.
export function upcomingByDay(events, now, { days = 7, throughWeekend = false, timeZone = BAND_TIMEZONE } = {}) {
  const { date: today, minutes } = zonedNow(now, timeZone);
  const last = windowEnd(today, { days, throughWeekend });
...
export function upcomingList(events, now, { days = 7, throughWeekend = false, limit = Infinity, timeZone = BAND_TIMEZONE } = {}) {
  const seen = new Set();
  const out = [];
  for (const day of upcomingByDay(events, now, { days, throughWeekend, timeZone })) {
