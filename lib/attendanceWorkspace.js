...
export async function loadProgramAttendanceWorkspace({ occurrenceKey = "", occurrenceKeys = null, studentId = "", includeCandidates = true } = {}) {
  const today = localDateAt(new Date(), ATTENDANCE_TIME_ZONE);
  const allEvents = await listAttendanceEventRows();
  // A scoped worker must never receive aggregate counts, concerns, or roster
  // details for events outside the occurrence explicitly authorized by the API.
  const allowedOccurrences = occurrenceKey
    ? new Set([occurrenceKey])
    : Array.isArray(occurrenceKeys)
      ? new Set(occurrenceKeys)
      : null;
  const events = allowedOccurrences
    ? allEvents.filter((event) => allowedOccurrences.has(event.occurrence_key))
    : allEvents;
  ...
  if (selectedSummary) {
    ...
    const { data: candidates, error: candidateError } = includeCandidates
      ? await supabaseAdmin
  ...
  if (studentId) {
    ...
    if (!studentRecord && !allowedOccurrences) {
