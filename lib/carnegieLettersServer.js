...
export function familyActor(session) {
  return { type: "parent", id: session.personId, name: session.email };
}

const LETTER_FIELDS = "id, portal_student_id, recipient_type, recipient_name, recipient_email, delivery_channel, meaning_text, help_text, template_version, status, version, approved_version, approved_at, review_note, submitted_at, printed_at, delivery_reported_at, created_at, updated_at";
...
// The students this signed-in adult may act for (#114). Like billing's trusted-guardian rule, but a
// guardian's roster-imported "legacy" link also qualifies, matching what the Family Portal already
// shows. Student self-records still need high assurance. Letters stay staff-reviewed and reported
// gifts stay staff-confirmed.
export async function notesStudentIds(personId) {
  if (!personId) return [];
  const { data, error } = await supabaseAdmin
  ...
  if (error) throw new Error("Students could not be loaded.");
  return [...new Set((data || []).filter(notesEligibleLink).map((row) => row.student_id).filter(Boolean))];
}

export async function familyStudents(personId) {
  return activeStudents(await notesStudentIds(personId));
}
...
export async function familyDashboard(personId) {
  ...
  return students.map((student) => {
    ...
    return {
      ...
      // Expected gifts staff recorded (#110) are pending in the same way. Family sees amount only.
      ...
      depositPaidCents: deposits.get(student.id) || 0,
      link: code ? { path: carnegieStudentPath(code), readable: carnegieStudentReadableUrl(code) } : null,
      letters: letters.filter((letter) => letter.portal_student_id === student.id)
    };
  });
}
...
export async function loadFamilyLetter(personId, letterId) {
  ...
  const allowed = await notesStudentIds(personId);
...
export async function createFamilyLetter(personId, body) {
...
export async function updateFamilyLetter(personId, letter, body) {
  ...
  const actor = { last_actor_type: "family", last_actor_id: String(personId) };
  try {
...
const REPORT_FIELDS = "id, portal_student_id, donor_name, donor_email, reported_amount_cents, reported_method, check_number, note, status, confirmed_amount_cents, confirmed_method, sponsor_gift_id, reject_reason, reviewed_at, created_at";
...
export async function createReportedGift(personId, body) {
  ...
  const { data, error } = await supabaseAdmin
    .from("carnegie_reported_gifts")
    .insert({ ...report, portal_student_id: studentId, reported_by_person_id: personId })
    .select(REPORT_FIELDS)
    .single();
