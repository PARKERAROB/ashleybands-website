...
import {
  ...
  carnegieSubmissionRequestsDeposit,
...
export function validateCarnegieSubmission(input, { staffVerbal = false } = {}) {
  ...
  const depositChoice = value(input.depositChoice, 40);
  const allowedDepositChoices = new Set(CARNEGIE_DEPOSIT_CHOICES.map((option) => option.value));
  if (response !== "no" && !allowedDepositChoices.has(depositChoice)) {
    return { error: "Choose whether you can pay the $50 conditional deposit at this time." };
  }
  ...
  return {
    ...
    depositChoice: response === "no" ? "" : depositChoice,
    helpOptions,
    guardianName,
    guardianEmail,
    guardianPhone,
...
export async function recordCarnegieSubmission({
  ...
  const { data, error } = await supabaseAdmin.rpc("record_carnegie_trip_submission", {
    ...
    p_deposit_choice: fields.depositChoice,
...
export async function carnegieSubmissionStatus(submission) {
  ...
  const depositRequested = carnegieSubmissionRequestsDeposit(submission);
  return {
    submissionId: submission.id,
    response: submission.response,
    source: submission.source,
    maximumFamilyAmountBand: submission.maximum_family_amount_band,
    depositChoice: submission.deposit_choice || "",
    depositRequested,
    agreementVersion: submission.agreement_version,
    createdAt: submission.created_at,
    signed: Boolean(submission.signed_at),
    paid: Boolean(payment),
    payment,
    checkoutToken: depositRequested && !payment
...
export async function latestCarnegieSubmissions(studentIds) {
  ...
  const results = await Promise.all(chunks(studentIds).map((part) => supabaseAdmin.from("carnegie_trip_submissions")
    .select("id,student_id,source,response,maximum_family_amount_band,deposit_choice,help_options,guardian_name,guardian_email,guardian_phone,guardian_signature,student_signature,agreement_version,terms_accepted,signed_at,note,created_at,submitted_by_staff_id")
    .in("student_id", part)
    .order("created_at", { ascending: false })));
...
export async function createCarnegiePaymentOrder(token) {
  ...
  const { data: submission, error: submissionError } = await supabaseAdmin.from("carnegie_trip_submissions")
    .select("id,student_id,response,deposit_choice,agreement_version,guardian_email")
    .eq("id", payload.submissionId).eq("student_id", payload.studentId).maybeSingle();
  const depositRequested = carnegieSubmissionRequestsDeposit(submission);
  if (submissionError || !submission || !depositRequested) throw new Error("A family deposit choice is required before payment.");
...
export async function loadCarnegieDashboard() {
  ...
  const rows = students.map((student) => {
    ...
    return {
      ...
      depositStatus: completed ? "received" : refunded ? "refunded" : activeCharge ? "payment_pending" : submission?.deposit_choice === "cannot_pay_now" ? "unable_now" : "not_requested",
