...
// ---- Family authorization (#114) -----------------------------------------------------------
// A trusted link row (already filtered to trusted relationships and active students) lets an adult
// act for the student when the adult is a guardian with legacy, medium or high assurance, or the
// student's own record with high assurance.
export const NOTES_GUARDIAN_ASSURANCE = Object.freeze(["legacy", "medium", "high"]);
export function notesEligibleLink(row) {
  ...
  return person?.person_type === "guardian" && NOTES_GUARDIAN_ASSURANCE.includes(row?.assurance_level);
}
