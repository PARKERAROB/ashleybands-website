...
import { readFamilyCookie } from "@/lib/familyAuthCookie";
...
export function readFamilySession(req) {
  // Prefer the httpOnly cookie; fall back to legacy x-family-* headers so already
  // signed-in localStorage sessions keep working during/after the cookie rollout.
  const cookie = readFamilyCookie(req);
  if (cookie?.id && cookie?.token) {
    return { familyId: cookie.id, token: cookie.token };
  }
  return {
