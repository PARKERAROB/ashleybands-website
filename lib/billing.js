...
export async function loadStudentLedgers(studentIds) {
  ...
  const [{ data: charges }, { data: payments }, { data: balances }] = await Promise.all([
    supabaseAdmin
      .from("fee_charges")
      .select("id, student_id, category, label, amount_cents, status, created_at")
      .in("student_id", ids)
      .order("created_at", { ascending: true }),
    supabaseAdmin
      .from("fee_payments")
      .select("id, student_id, amount_cents, method, status, category, received_at, notes, created_at")
      .in("student_id", ids)
      .order("created_at", { ascending: true }),
    supabaseAdmin
