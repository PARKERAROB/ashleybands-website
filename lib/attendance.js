...
async function loadStaffAttendance(eventId) {
  const [membersResult, observationsResult] = await Promise.all([
    supabaseAdmin
      .from("staff")
  ...
  if (membersResult.error || observationsResult.error) {
    throw membersResult.error || observationsResult.error;
  }
...
export async function getAttendanceSheet({ occurrenceKey, session, now = new Date() }) {
  ...
  const [observationsResult, exceptionsResult, staff] = await Promise.all([
    supabaseAdmin
    ...
    loadStaffAttendance(selected.id)
  ]);
...
export function attendanceAuditTables() {
  return "portal_students,staff,attendance_events,attendance_observations,attendance_exceptions,attendance_staff_observations";
}
