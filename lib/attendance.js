...
export async function getAttendanceSheet({ occurrenceKey, session, now = new Date() }) {
  ...
  return {
    ...
    staff,
    exceptions,
    canWriteAttendance: Boolean(session?.permissions?.eventsWrite)
      && writable
      && (!selected.correctionOpenedAt || session?.access === "staff"),
    ...
    canPrepare: Boolean(session?.permissions?.prepare)
      && !selected.isPast
      && !selected.rosterLockedAt
      && ["scheduled", "prepared"].includes(selected.lifecycleState),
    canComplete: Boolean(session?.permissions?.complete)
      && writable
      && students.length > 0
...
export async function updateAttendanceObservation({ occurrenceKey, studentId, changes, actorStaffId, now = new Date() }) {
  ...
  if (selected.correctionOpenedAt) {
    if (!actorStaffId) {
      const forbidden = new Error("Named staff access is required to correct completed attendance.");
