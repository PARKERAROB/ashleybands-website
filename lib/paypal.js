...
// Read-only reconciliation. This never creates, captures or refunds a payment.
export function createPaypalFundingReader() {
  let token;
  async function read(path) {
    if (PAYPAL_ENV !== "live") throw new Error("Live PayPal required for public funding");
    token ||= getAccessToken(AbortSignal.timeout(15000));
    const response = await fetch(`${paypalBaseUrl()}${path}`, {
      headers: { Authorization: `Bearer ${await token}` },
      cache: "no-store",
      signal: AbortSignal.timeout(15000)
    });
    if (!response.ok) throw new Error("Processor reconciliation unavailable");
    return response.json();
  }
  return {
    capture: id => read(`/v2/payments/captures/${encodeURIComponent(id)}`),
    async refunds(orderId, captureId) {
      if (!orderId) throw new Error("Refund order missing");
      const order = await read(`/v2/checkout/orders/${encodeURIComponent(orderId)}`);
      if (order.id !== orderId) throw new Error("Refund order mismatch");
      const units = (order.purchase_units || []).filter(unit =>
        unit.payments?.captures?.some(capture => capture.id === captureId));
      if (units.length !== 1 || units[0].payments.captures.length !== 1) throw new Error("Ambiguous refund allocation");
      const refunds = units[0].payments.refunds || [];
      return Promise.all(refunds.map(refund => read(`/v2/payments/refunds/${encodeURIComponent(refund.id)}`)));
    }
  };
}
