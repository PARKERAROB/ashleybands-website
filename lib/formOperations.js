...
const CARNEGIE_FILTER_PREFIX_BY_GROUP = {
...
export async function loadFormOperations() {
  ...
  const [statusResult, onboardingResult, requestResult, groupResult, membershipResult, musicProfileResult, carnegieResult] = await Promise.all([
    ...
    studentIds.length
      ...
      : Promise.resolve({ data: [], error: null }),
    groupIds.length
      ? supabaseAdmin.from("program_groups")
        .select("id,code")
        .in("id", groupIds)
      : Promise.resolve({ data: [], error: null }),
    groupIds.length
    ...
    studentIds.length
      ...
      : Promise.resolve({ data: [], error: null }),
    studentIds.length
      ? supabaseAdmin.from("carnegie_trip_submissions")
        .select("id,student_id,source,response,agreement_version,signed_at,created_at")
        .in("student_id", studentIds)
        .order("created_at", { ascending: false })
      : Promise.resolve({ data: [], error: null }),
  ]);
  for (const result of [statusResult, onboardingResult, requestResult, groupResult, membershipResult, musicProfileResult, carnegieResult]) {
  ...
  const groupById = new Map((groupResult.data || []).map((row) => [row.id, row]));
  const carnegieByStudent = new Map();
  for (const row of carnegieResult.data || []) if (!carnegieByStudent.has(row.student_id)) carnegieByStudent.set(row.student_id, row);
  ...
  for (const requirement of requirements) {
    ...
    for (const student of applicable) {
      ...
      if (definition.code === "carnegie-2027-initial-intent") {
        const submission = carnegieByStudent.get(student.id);
        if (submission) derived = {
          state: submission.source === "staff_verbal" ? "needs_review" : "complete",
          sourceRef: `carnegie_trip_submissions:${submission.id}`,
          submittedAt: submission.created_at,
          completedAt: submission.signed_at,
          updatedAt: submission.created_at,
          nextAction: submission.source === "staff_verbal" ? "Help family sign in and complete the signed response" : "",
          actionHref: "/admin/carnegie-2027",
          fulfillmentLabel: submission.source === "staff_verbal" ? "Verbal response; signature follow-up open" : "Signed family response",
          systemOwned: true,
        };
      }
      ...
      rows.push({
        ...
        recordedResponse: definition.code === "carnegie-2027-initial-intent"
          ? carnegieByStudent.get(student.id)?.response || ""
          : "",
        recordedAgreementVersion: definition.code === "carnegie-2027-initial-intent"
          ? carnegieByStudent.get(student.id)?.agreement_version || ""
          : "",
  ...
  const definitions = requirements.map((requirement) => {
    const definition = one(requirement, "form_definitions");
    const version = one(requirement, "form_versions");
    const carnegiePrefix = definition?.code === "carnegie-2027-initial-intent"
      ? CARNEGIE_FILTER_PREFIX_BY_GROUP[groupById.get(requirement.scope_ref)?.code]
      : "";
    return definition && version ? {
      requirementId: requirement.id,
      code: definition.code,
      title: carnegiePrefix ? `${carnegiePrefix} - Carnegie Commitment` : definition.title,
      deliveryType: version.delivery_type,
      schoolYear: requirement.school_year,
