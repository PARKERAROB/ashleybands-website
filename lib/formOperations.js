  ...
  const [statusResult, onboardingResult, requestResult, membershipResult, musicProfileResult] = await Promise.all([
    ...
    studentIds.length
      ? supabaseAdmin.from("portal_instrument_requests")
        .select("id,student_id,status,school_year,submitted_at,updated_at")
        .in("student_id", studentIds)
      : Promise.resolve({ data: [], error: null }),
    groupIds.length
  ...
  for (const requirement of requirements) {
    ...
    for (const student of applicable) {
      ...
      if (definition.code === "county-instrument-agreement") {
        ...
        if (request) derived = {
          state: request.status === "assigned" ? "complete" : request.status === "submitted" ? "submitted" : "not_started",
          sourceRef: `portal_instrument_requests:${request.id}`,
          submittedAt: request.submitted_at,
          completedAt: request.status === "assigned" ? request.updated_at : null,
          updatedAt: request.updated_at || request.submitted_at,
          nextAction: request.status === "submitted" ? "Review instrument request" : "",
          systemOwned: true,
        };
      }
      ...
      rows.push({
        ...
        actionHref: version.delivery_type === "portal" ? "" : version.action_href,
