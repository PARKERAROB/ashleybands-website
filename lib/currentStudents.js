...
export async function loadStudent360(studentId, access = {}) {
  const allowed = {
    assets: access.assets !== false,
    attendance: access.attendance !== false,
    finance: access.finance !== false,
    forms: access.forms !== false,
    memberships: access.memberships !== false,
    communications: access.communications !== false,
  };
  ...
  const [resourcesResult, assetsResult, requestsResult, attendanceResult, sponsorResult, profileResult, musicResult, otherInstrumentsResult, interestsResult, enrollmentResult, backgroundResult, supportResult, completionResult, statusHistoryResult, membershipsResult, ledgers, formsData] = await Promise.all([
    allowed.assets ? supabaseAdmin.from("portal_student_resources")
      .select("locker_number,tuner_number,tuner_shared_with,assignment_status,updated_at")
      .eq("student_id", studentId)
      .maybeSingle() : Promise.resolve({ data: null, error: null }),
    allowed.assets ? supabaseAdmin.from("asset_assignments")
      .select("id,starts_at,assignment_status,source_system,assets(id,asset_type,asset_tag,display_name,operational_status,condition_summary,asset_instruments(instrument_type,brand,model,model_markings,serial_number))")
      .eq("student_id", studentId)
      .is("ends_at", null)
      .in("assignment_status", ["current", "provisional"]) : Promise.resolve({ data: [], error: null }),
    allowed.assets ? supabaseAdmin.from("portal_instrument_requests")
      .select("id,status,school_year,submitted_at,updated_at")
      .eq("student_id", studentId)
      .order("submitted_at", { ascending: false }) : Promise.resolve({ data: [], error: null }),
    allowed.attendance ? supabaseAdmin.from("attendance_observations")
      .select("status,arrived_at,departed_at,updated_at,attendance_events(id,title,starts_at,occurrence_key)")
      .eq("portal_student_id", studentId) : Promise.resolve({ data: [], error: null }),
    allowed.finance ? supabaseAdmin.from("sponsor_gifts")
    ...
    supabaseAdmin.from("portal_onboarding_completions")
      ...
      .limit(1)
      .maybeSingle(),
    supabaseAdmin.from("portal_student_status_events")
      .select("id,from_status,to_status,reason,effective_at,changed_by,source")
      .eq("student_id", studentId)
      .order("effective_at", { ascending: false }),
    student.status === "active" && allowed.memberships ? supabaseAdmin.from("program_memberships")
    ...
    allowed.finance ? loadStudentLedgers([studentId]) : Promise.resolve({ charges: {}, payments: {}, balances: {} }),
    student.status === "active" && allowed.forms ? loadFormOperations() : Promise.resolve({ rows: [] }),
  ]);

  for (const result of [resourcesResult, assetsResult, requestsResult, attendanceResult, sponsorResult, profileResult, musicResult, otherInstrumentsResult, interestsResult, enrollmentResult, backgroundResult, supportResult, completionResult, statusHistoryResult, membershipsResult]) {
    if (result.error) throw new Error(result.error.message);
  }

  const charges = ledgers.charges[studentId] || [];
  const payments = ledgers.payments[studentId] || [];
  const isCampaign = (category, kind = "") => kind === "funding_goal" || chargeKindForCategory(category) === "funding_goal";
  const sum = (rows) => rows.reduce((total, row) => total + (Number(row.amount_cents) || 0), 0);
  const feeCharges = charges.filter((row) => row.status === "active" && !isCampaign(row.category, row.kind));
  const feePayments = payments.filter((row) => row.status === "completed" && !row.is_sponsorship && !isCampaign(row.category, row.kind));
  const campaignCharges = charges.filter((row) => row.status === "active" && isCampaign(row.category, row.kind));
  const campaignContributions = payments.filter((row) => row.status === "completed" && !row.is_sponsorship && isCampaign(row.category, row.kind));
  const legacySponsorshipCredits = payments.filter((row) => row.status === "completed" && row.is_sponsorship && isCampaign(row.category, row.kind));
  ...
  const assets = (assetsResult.data || []).map((assignment) => {
    const asset = one(assignment, "assets");
    if (!asset) return null;
    const instrument = one(asset, "asset_instruments");
    return {
      id: asset.id,
      ...
      condition: asset.condition_summary,
      assignmentStatus: assignment.assignment_status,
      assignedAt: assignment.starts_at,
      instrument,
    };
  }).filter(Boolean);
  const instruments = assets.filter((asset) => asset.assetType === "instrument").map((asset) => ({
    id: asset.id,
    instrument_type: asset.instrument?.instrument_type || "",
    brand: asset.instrument?.brand || "",
    model_markings: asset.instrument?.model || asset.instrument?.model_markings || "",
    serial_number: asset.instrument?.serial_number || "",
    issued_at: asset.assignedAt,
    issued_condition: asset.condition,
    review_status: asset.assignmentStatus,
  }));
  const studentForms = (formsData.rows || []).filter((row) => row.student.id === studentId);
  const formSummary = studentForms.reduce((result, row) => {
    result.total += 1;
    if (["complete", "waived", "not_required"].includes(row.state)) result.complete += 1;
    else result.action += 1;
    return result;
  }, { total: 0, action: 0, complete: 0 });
  const feeChargedCents = sum(feeCharges);
  const feePaidCents = sum(feePayments);
  const campaignGoalCents = sum(campaignCharges);
  const campaignRaisedCents = sum(campaignContributions) + confirmedSponsorshipCents;
  ...
  return {
    ...
    recentAttendance: attendanceRows
      ...
      .slice(0, 5),
    finances: allowed.finance ? {
      feeChargedCents,
      feePaidCents,
      feeBalanceCents: feeChargedCents - feePaidCents,
      campaignGoalCents,
      campaignRaisedCents,
      campaignRemainingCents: Math.max(campaignGoalCents - campaignRaisedCents, 0),
    ...
    programMemberships,
    forms: allowed.forms ? { available: studentForms.length > 0, ...formSummary } : null,
    access: allowed,
  };
}
