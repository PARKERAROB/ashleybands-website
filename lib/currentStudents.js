...
function buildStudent(student, guardians, studentContacts, openInstrumentRequest) {
...
export async function loadCurrentStudents(view = "active") {
  ...
  const [{ guardiansByStudent, studentContactsByStudent }, openRequests] = await Promise.all([
    loadPeople(ids),
...
export async function loadStudent360(studentId) {
  ...
  const [resourcesResult, instrumentsResult, requestsResult, attendanceResult, sponsorResult, ledgers] = await Promise.all([
  ...
  for (const result of [resourcesResult, instrumentsResult, requestsResult, attendanceResult, sponsorResult]) {
