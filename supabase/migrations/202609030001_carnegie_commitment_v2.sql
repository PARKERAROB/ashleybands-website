-- Carnegie family-intent v2 separates willingness to travel from financial capacity.
-- Existing v1 submissions remain append-only and keep their original stored values.
-- provenance: response and maximum-family-amount values are supplied by a family or recorded by staff from a family response.

alter table public.carnegie_trip_submissions
  drop constraint if exists carnegie_trip_submissions_maximum_family_amount_band_check;

alter table public.carnegie_trip_submissions
  add constraint carnegie_trip_submissions_maximum_family_amount_band_check
  check (maximum_family_amount_band in (
    '',
    '500_or_less',
    'full_assistance_required',
    'up_to_500',
    '501_1000',
    '1001_1500',
    '1501_1999'
...
begin
  ...
  if p_agreement_version not in ('2026-09-01-v1','2026-09-03-v2') then raise exception 'invalid agreement version'; end if;
  ...
  if p_response = 'interested_limited' and (
    ...
    or
    (p_agreement_version = '2026-09-03-v2' and p_maximum_family_amount_band not in ('full_assistance_required','up_to_500','501_1000','1001_1500','1501_1999'))
  ) then
