...
export async function POST(request) {
  try {
    ...
    const res = await fetch(`${url}/rest/v1/rpc/staff_sprint_submit_answer`, {
      method: "POST",
      headers: supabaseHeaders(key, { "Content-Type": "application/json" }),
      body: JSON.stringify({ p_player_id: playerId, p_is_correct: isCorrect })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return Response.json({ error: data.message || "answer failed" }, { status: 500 });
