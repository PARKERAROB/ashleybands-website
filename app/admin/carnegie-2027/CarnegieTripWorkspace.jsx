...
function CarnegieTripWorkspaceContent() {
  ...
  function downloadCsv() {
    const columns = ["Student","Ensemble","Response","Source","Submitted","Guardian","Email","Phone","Maximum amount","Help offered","Deposit","Follow-up","Eligibility","Staff note"];
    const quote = (input) => `"${String(input ?? "").replaceAll('"','""')}"`;
    const lines = rows.map((row) => [
      row.student.display_name, row.student.ensemble_2026, carnegieResponseLabel(row.submission?.response), sourceLabels[row.submission?.source] || "",
      row.submission?.created_at || "", row.contact.name, row.contact.email, row.contact.phone,
      carnegieAmountBandLabel(row.submission?.maximum_family_amount_band), (row.submission?.help_options || []).map((value) => CARNEGIE_HELP_OPTIONS.find((item) => item.value === value)?.label || value).join("; "),
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.filters} aria-label="Filter and sort">
        ...
        <label>Response<select value={responseFilter} onChange={(event) => setResponseFilter(event.target.value)}><option value="all">All responses</option>{CARNEGIE_RESPONSE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}<option value="missing">No response</option></select></label>
...
function TripRow({ row, onChanged, setMessage }) {
  ...
  return <tr>
    ...
    <td>{row.submission ? <><strong>{carnegieResponseLabel(row.submission.response)}</strong><span>{sourceLabels[row.submission.source]} · {displayDate(row.submission.created_at)}</span>{row.submission.source === "staff_verbal" ? <em>Unsigned verbal record</em> : <em>Signed family response</em>}</> : <em>No response</em>}</td>
    ...
    <td><strong>{row.contact.name || "No guardian listed"}</strong><span>{row.contact.email || "No email"}</span><span>{row.contact.phone || "No phone"}</span>{row.submission ? <details><summary>All form inputs</summary><p><b>Maximum:</b> {carnegieAmountBandLabel(row.submission.maximum_family_amount_band) || "Not applicable"}</p><p><b>Help:</b> {help.join("; ") || "None selected"}</p><p><b>Guardian signature:</b> {row.submission.guardian_signature || "Verbal — not signed"}</p><p><b>Student signature:</b> {row.submission.student_signature || "Verbal — not signed"}</p>{row.submission.note ? <p><b>Submission note:</b> {row.submission.note}</p> : null}</details> : null}</td>
...
function VerbalForm({ rows, onSaved }) {
  ...
  const [response, setResponse] = useState("serious_yes");
