...
function Orders({ session }) {
  ...
  useEffect(()=>{fetch("/api/admin/clothing-orders").then(async r=>{const b=await r.json();if(!r.ok)throw new Error(b.error);setOrders(b.orders||[])}).catch(e=>setError(e.message))},[session]);
