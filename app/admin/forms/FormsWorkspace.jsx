...
function LiveForms({ session, signOut, initialStudentId, initialView }) {
  ...
  const rows = useMemo(() => {
    ...
    const visible = (data?.rows || []).filter((row) => {
      ...
      if (term && ![studentName(row), row.student.displayName, row.definition.title, row.stateLabel, row.source, row.recordedResponse ? carnegieResponseLabel(row.recordedResponse) : ""].join(" ").toLowerCase().includes(term)) return false;
      return true;
    });
  ...
  const metricRows = (data?.rows || []).filter((row) => {
    ...
    if (status && row.state !== status) return false;
    if (delivery && row.deliveryType !== delivery) return false;
    if (search.trim() && ![studentName(row), row.student.displayName, row.definition.title, row.stateLabel, row.source, row.recordedResponse ? carnegieResponseLabel(row.recordedResponse) : ""].join(" ").toLowerCase().includes(search.trim().toLowerCase())) return false;
    return true;
  });
