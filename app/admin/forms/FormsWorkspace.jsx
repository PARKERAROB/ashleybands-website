...
function FormRow({ row, onOpenStudent, onUpdate, saving }) {
  ...
  return <tr><td data-label="Student"><button className={styles.student} onClick={onOpenStudent}><strong>{studentName(row)}</strong><span>{row.student.grade}</span></button></td><td data-label="Requirement"><strong>{row.definition.title}</strong><small>{row.source} · {row.schoolYear}</small></td><td data-label="Status"><span className={`${styles.status} ${styles[tone]}`}>{row.stateLabel}</span></td><td data-label="Completion">{row.deliveryType.replace("staff_record", "staff record")}</td><td data-label="Next action"><div className={styles.actions}>{row.actionHref && !COMPLETE.has(row.state) ? <Link href={row.actionHref}>{row.nextAction || "Open form"}</Link> : null}{row.systemOwned ? <span>{row.nextAction || (COMPLETE.has(row.state) ? "Recorded by connected workflow" : "Waiting on family workflow")}</span> : <select aria-label={`Update ${row.definition.title} for ${row.student.displayName}`} disabled={saving} value={row.state} onChange={(event) => onUpdate(row, event.target.value)}><option value="not_started">Not started</option><option value="submitted">Submitted</option><option value="needs_review">Needs review</option><option value="needs_correction">Needs correction</option><option value="complete">Complete</option><option value="waived">Waived</option><option value="not_required">Not required</option></select>}</div></td></tr>;
}
