  ...
  return <div className={styles.overview}>
    <div>
      <section className={styles.panel}>
        ...
        {mine.slice(0, 4).map(row => <button className={styles.attentionRow} key={row.id} onClick={() => navigate("commitments")}><span className={styles.rowIcon} aria-hidden="true">✓</span><span><strong>{row.title}</strong><small>{row.due ? `Due ${row.due}` : "No date set"}</small></span><span className={styles.badge}>{row.status}</span></button>)}
